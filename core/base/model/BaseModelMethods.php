...
abstract class BaseModelMethods
{
		...
		if (!empty($set['where']) && is_array(($set['where']))) {
			...
			foreach ($set['where'] as $key => $item) {
				...
				if ($operand === 'IN' || $operand === 'NOT IN') {
					...
					if ($item !== null && is_string($item) && strpos($item, 'SELECT') === 0) {
	...
	protected function createInsert($fields, $files, $except)
	{
		...
		// массив $insert_arr- это ассоциативный массив, который возвращает массив
		// с ячейками:
		// fields  - строка с названиями полей
		// values  - строка со значениями для вставки
		//
		// Сразу создаём оба ключа, так как PHP 8.3 не допускает
		// конкатенацию к несуществующему элементу массива
		$insert_arr = [
			'fields' => '(',
			'values' => '',
		];
		...
		// Определим пришёл в fields массив с числовыми ключами или со строковыми
		// array_key_first() безопаснее, чем array_keys($fields)[0],
		// так как не вызовет Undefined array key 0 при пустом массиве
		$array_type = array_key_first($fields);
		...
		if (is_int($array_type)) {
			...
			foreach ($fields as $i => $item) {
				...
				// PHP 8.3:
				// values уже существует, поэтому безопасно добавляем данные
				$insert_arr['values'] .= '(';
